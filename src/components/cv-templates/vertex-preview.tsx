...
export function VertexPreview({ resume }: { resume: ResumeData }) {
  ...
  return (
    ...
    >
      {/* Compact header */}
      <header className="mb-5 border-b-2 border-[#0d9488] pb-4">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h1 className="text-[28px] font-bold leading-none tracking-tight">{fullName}</h1>
            <p className="mt-1.5 text-[11.5px] font-bold uppercase tracking-[0.16em] text-[#0d9488]">
              {resume.title || "Professional Title"}
            </p>
          </div>
          <div className="text-right text-[9.5px] leading-[1.55] text-[#475569]">
            {contact.length === 0 ? (
      ...
      {resume.summary && (
        <section className="mb-5">
          ...
          <p className="text-[10px] leading-[1.55] text-[#1e293b]">{resume.summary}</p>
        </section>
      )}
                ...
                {resume.experiences.map((exp) => (
                  <div key={exp.id}>
                    <div className="flex flex-wrap items-baseline justify-between gap-x-3">
                      <h3 className="text-[11.5px] font-bold text-[#0f172a]">
                        {exp.role || "Role Title"}
                        <span className="font-normal text-[#0d9488]"> · {exp.company || "Company"}</span>
                      </h3>
                      <span className="text-[9.5px] font-semibold text-[#475569]">
                        {formatDateRange(exp.startDate, exp.endDate, exp.current)}
                      </span>
                    </div>
                    {exp.location && (
                      <p className="text-[9.5px] italic text-[#64748b]">{exp.location}</p>
                    )}
                    <ul className="mt-1.5 space-y-0.5">
                      {exp.bullets.filter(Boolean).map((bullet, i) => (
                        <li key={i} className="relative pl-3 text-[10px] leading-[1.5] text-[#1e293b]">
                          <span className="absolute left-0 top-[6px] h-[3px] w-[3px] rounded-full bg-[#0d9488]" />
                          {bullet}
                        </li>
                      ))}
                    </ul>
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <h2 className="mb-2 text-[9.5px] font-bold uppercase tracking-[0.22em] text-[#0d9488]">
      {children}
    </h2>
  );
}
